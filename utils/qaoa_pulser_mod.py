...
class qaoa_pulser(object):
    ...
    def generate_training_data(self, N_points, depth, extreme_params):
        X, Y, VAR, FID, FID_EXACT, SOL_RATIO = [], [], [], [], []
        ...
        X = X.tolist()
        for x in X:
            y, var_y, fid, fid_ex, sol_ratio = self.apply_qaoa(x)
            Y.append(y)
            VAR.append(var_y)
            FID.append(fid)
            FID_EXACT.append(fid_ex)
            SOL_RATIO.append(sol_ratio)
        
            return X, Y, VAR, FID, FID_EXACT, SOL_RATIO

    def fidelity_gs_sampled(self, C, solution_ratio = False):
        '''
    ...
    def fidelity_gs_exact(self, last_evolution_state):
        '''
        ...
        fid = fidelity(self.gs_state, last_evolution_state)
        return fid
    ...
    def apply_qaoa(self, params):
        results, evol_states = self.quantum_loop(params)
        
        fidelity = self.fidelity_gs_sampled(results)
        fidelity_exact = self.fidelity_gs_exact(evol_states[-1])
        sol_ratio = self.solution_ratio(results)
        energy, variance = self.expected_energy_and_variance(results)
        
        return energy, variance, fidelity, fidelity_exact, sol_ratio
